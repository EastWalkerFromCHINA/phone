...
table_csv = "pi_xie.csv" #仅需在此处修改要分析的文件
...
with left_column:
    st.subheader("最大销量:")
    st.subheader(f"{number_max} 件/月")
with middle_column:
    st.subheader("平均销量:")
    st.subheader(f"{nunber_mean} 件/月")
with right_column:
    st.subheader("平均价格:")
...
# 各地区商品销量(柱状图)
sales_by_product = df_selection.groupby(by=["address"]).sum()[["number_purchaser"]].sort_values(by="number_purchaser") # 对可交互数据集按地区销量进行计数，排序,sort是排序
...
fig_product_sales = px.bar(
    ...
    x = sales_by_product.index, # x坐标，为数据集的索引,为上方by参数
...
# 排名前十店铺销量
store_by_product = df_selection.groupby(by=["name_store"]).sum()[["number_purchaser"]].sort_values(by="number_purchaser")[-11:-1] # 对可交互数据集按店销量进行计数，排序

store_product_sales = px.bar(
    store_by_product ,   # 这里是处理过的可交互数据集
    y = 'number_purchaser',  # y坐标，为可交互数据集的值
    x = store_by_product.index, # x坐标，为数据集的索引,为上方by参数
    title="<b>销量前十门店</b>", # 表标题
    text = 'number_purchaser',  # 柱子内标注，最好是y轴值
    color_discrete_sequence=["#0083B8"] * len(store_by_product),
    template="plotly_white",
)
# 添加坐标注释
store_product_sales.update_layout(xaxis_title='店铺',
                 yaxis_title='销量')

#柱形图文字格式
store_product_sales.update_traces(
                 textposition='outside',
                 texttemplate='%{text:,.2s}')

st.plotly_chart(store_product_sales)  # 该代码仅用于plotly画图方法
#store_product_sales.show() # 用于检验图表
